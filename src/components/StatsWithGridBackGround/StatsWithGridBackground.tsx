...
export const Grid = ({
  ...
  // Générer un pattern avec coordonnées uniques ou utiliser celui fourni
  const p = pattern ?? (() => {
    // Ensemble pour suivre les coordonnées déjà utilisées
    const usedCoords = new Set();
    const result = [];
    
    for (let i = 0; i < 5; i++) {
      let x, y, coordKey;
      // Continuer à générer des coordonnées jusqu'à en trouver d'uniques
      do {
      ...
      // Marquer les coordonnées comme utilisées
